...
const CommentsAndRatings = forwardRef((props, ref) => {
  const notify = (msg) =>
    toast.success(msg, {
      icon: false,
      toastId: "success",
  ...
  const handlerOpen = (editOrCreate, Id, commentIdp) => {
    setRating(0);
    setHover(0);
    reset();
    setIsOpen(!isOpen);
    setIsEdit(editOrCreate);
    setProductId(Id);
    setCommentId(commentIdp);
  };
  ...
  const onSubmit = async (form) => {
    if (isEdit === false) {
      if (rating < 0 || rating > 5 || rating === null) {
        notifyError(
          "completa todos los campos para poder enviar tu comentario"
        );
      } else {
      ...
      if (rating < 0 || rating > 5 || rating === null) {
        notifyError(
          "completa todos los campos para poder editar tu comentario"
        );
      } else {
        const response = await axios.put(
          `/commentsRating/updateCommentsRating/${commentId}`,
          {
            comments: form.comment,
            rating: parseInt(rating),
            UserId: user?.sub,
        ...
        reset();
        setRating(0);
        notify("Comentario editado correctamente");
        props.fetchData();
        setIsOpen(!isOpen);
      }
    }
